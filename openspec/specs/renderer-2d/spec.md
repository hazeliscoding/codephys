...
Provide a renderer-agnostic 2D drawing layer: batched primitives (lines, circles,
polygons, arrows), an orthographic `Camera2D` with pan/zoom and world↔screen conversion,
and embedded bitmap-font text. All OpenGL state stays inside `render`/`platform` so scenes
and UI never touch GL directly.

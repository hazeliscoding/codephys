...
Provide the renderer-agnostic simulation core: a `Particle` point mass, a `World` step
pipeline, four switchable numerical integrators, per-tick conserved-quantity reporting,
and deterministic trajectories. The core depends only on the standard library and
`physics::math`, keeping it headlessly testable and reusable.

...
Provide the hand-rolled math foundation for CodePhys: `constexpr` vector/matrix/quaternion
value types, SI physical constants, and centralized unit conversions. These types depend
only on the C++ standard library so they are headlessly testable and shareable with the
renderer via the `physics::math` dependency edge, without `physics` depending on `render`.
